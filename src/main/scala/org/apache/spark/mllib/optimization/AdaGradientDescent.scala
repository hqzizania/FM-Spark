...
class AdaGradientDescent private[spark] (
  ...
  def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = AdaGradientDescent.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
...
object AdaGradientDescent extends Logging {
  ...
  def runMiniBatchSGD(
    ...
    var regVal = updater.compute(
      weights, Vectors.zeros(weights.size), m, v, beta1Pow, beta2Pow, 1, regParam)._6
  ...
  def runMiniBatchSGD(
    ...
    AdaGradientDescent.runMiniBatchSGD(data, gradient, updater, stepSize, numIterations,
